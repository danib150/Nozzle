...
versionjson="$workdir/work/mc-dev/$minecraftversion.json"
...
if [ ! -d "$spigotdecompiledir/net" ]; then
    ...
    cd "$basedir"
    set +e
    java -jar "$workdir/BuildData/bin/fernflower.jar" -dgs=1 -hdc=0 -rbr=0 -asc=1 -udv=0 "$classdir" "$spigotdecompiledir"
    if [ "$?" != "0" ]; then
